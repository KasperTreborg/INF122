    ...
    cellRange dim ran
        = Set.fromList [Cell columns rows | columns <- validColumns, rows <- validRows]
          where
            ...
            startCol = column $ upperLeft ran
            endCol = column $ lowerRight ran
            startRow = row $ upperLeft ran
            endRow = row $ lowerRight ran
